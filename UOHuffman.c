...
void Compress(char *dest, const char *src, int *dest_size, int *src_size)
{
	...
	LogPrint("Compress(0x%x, 0x%x, %d, %d)", (int)dest, (int)src, *dest_size, *src_size);
...
void Decompress(char *dest, const char *src, int *dest_size, int *src_size, HuffmanObj *obj)
{
    ...
    LogPrint("Decompress(0x%x, 0x%x, %d, %d, 0x%x)", (int)dest, (int)src, *dest_size, *src_size, (int)obj);
	while(src_pos < *src_size)
	{
...
{
	LogPrint("DecompressClean(0x%x)", (int)obj);
